...
module aais::agent_identity {
    ...
    const COLLECTION_NAME: vector<u8> = b"AAIS Agent Identities";
    const COLLECTION_DESCRIPTION: vector<u8> = b"Verifiable on-chain identities for AI agents on Ascent";
    const COLLECTION_URI: vector<u8> = b"https://ascent.dev/agents/collection";
    ...
    struct AgentIdentity has key {
        ...
        /// AAIS reputation score (scaled by 100)
        aaiss_score: u64,
    }
    ...
    #[event]
    struct AAISScoreUpdated has drop, store {
        agent_id: String,
        new_score: u64,
        timestamp: u64,
    }
    ...
    public entry fun mint_identity(
        ...
        vector::push_back(&mut registry.agents, token_addr);
    ...
    /// Update AAIS score (called by reputation module)
    public entry fun update_aais_score(
        token_address: address,
        new_score: u64,
    ) acquires AgentIdentity {
        let identity = borrow_global_mut<AgentIdentity>(token_address);
    ...
    #[view]
    public fun get_identity(token_address: address): (String, String, String, bool, u64, u64) acquires AgentIdentity {
    ...
    #[view]
    public fun get_aais_score(token_address: address): u64 acquires AgentIdentity {
        let identity = borrow_global<AgentIdentity>(token_address);
        identity.aaiss_score
    }

    #[view]
    public fun get_all_agents(): vector<address> acquires IdentityRegistry {
        let registry = borrow_global<IdentityRegistry>(@aais);
        registry.agents
    }
}
