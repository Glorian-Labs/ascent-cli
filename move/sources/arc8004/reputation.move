...
module aaas::reputation {
    ...
    struct ReputationAttestation has store, drop, copy {
        ...
        payment_hash: String,
        /// Service rating (specific to the transaction)
        service_rating: u8,
    ...
    struct AgentScore has store, drop, copy {
        ...
        last_updated: u64,
        /// Total x402 payment volume (in USDC atomic units)
        total_volume: u64,
    }
    ...
    struct ReputationRegistry has key {
        /// Admin address
        admin: address,
        /// Attestations indexed by client address
        attestations: Table<address, vector<ReputationAttestation>>,
        /// Aggregated scores per agent (keyed by agent_id)
    ...
    public entry fun attest_feedback(
        ...
        assert!(service_rating >= 1 && service_rating <= 5, E_INVALID_SCORE);
        ...
        // Store attestation
        let registry = borrow_global_mut<ReputationRegistry>(@aais);
        ...
        agent_score.total_volume = agent_score.total_volume + payment_amount;
        ...
        event::emit(AgentScoreUpdated {
            ...
            total_volume: agent_score.total_volume,
            timestamp: now,
        });
    }

    /// Check if agent meets AgentMesh marketplace threshold
    #[view]
    public fun meets_marketplace_threshold(agent_id: String): bool acquires ReputationRegistry {
        let registry = borrow_global<ReputationRegistry>(@aais);
        ...
        score.average_score_scaled >= AGENTMARKETPLACE_MIN_SCORE
    }
    ...
    #[view]
    public fun get_agent_score(agent_id: String): (u64, u64, u64, u8, u64, u64) acquires ReputationRegistry {
        let registry = borrow_global<ReputationRegistry>(@aais);
    ...
    #[view]
    public fun get_trust_level_label(level: u8): String {
        if (level == TRUST_LEVEL_UNKNOWN) {
            std::string::utf8(b"Unknown")
        } else if (level == TRUST_LEVEL_NEW) {
            std::string::utf8(b"New")
        } else if (level == TRUST_LEVEL_DEVELOPING) {
            std::string::utf8(b"Developing")
        } else if (level == TRUST_LEVEL_ESTABLISHED) {
            std::string::utf8(b"Established")
        } else if (level == TRUST_LEVEL_TRUSTED) {
            std::string::utf8(b"Trusted")
        } else if (level == TRUST_LEVEL_EXCELLENT) {
            std::string::utf8(b"Excellent")
        } else {
            std::string::utf8(b"Invalid")
        }
    }
}
