    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (textTags.includes(el.tagName)) {
        ...
        if (hasBg || hasBorder || hasShadow) {
          errors.push(`Text element <${el.tagName.toLowerCase()}> has ${hasBg ? 'background' : hasBorder ? 'border' : 'shadow'}. Backgrounds, borders, and shadows are only supported on <div> elements.`);
          return;
        }
      }
      ...
      if (isContainer) {
        ...
        for (const node of el.childNodes) {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (text) {
              errors.push(`DIV element contains unwrapped text "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}". All text must be wrapped in <p>, <h1>-<h6>, <ul>, or <ol> tags.`);
            }
          }
        }
