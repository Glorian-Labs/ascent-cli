...
class MultiWalletTester {
  async testSingle(options) {
    ...
    try {
      ...
      // 1. Initial Request
      const endpoint = options.endpoint || 'http://localhost:3006/api/paid-endpoint';
      
      // Pre-flight check
      try {
        const healthRes = await fetch('http://localhost:3006/api/paid-endpoint', { method: 'POST', timeout: 5000 });
        if (healthRes.status === 404) {
          spinner.fail('Endpoint /api/paid-endpoint returned 404. Check your server routes.');
      ...
      // Simple pre-signing check to see if we can reach the node
      ...
      console.log(`- Payment signature generated: ${paymentSignature.slice(0, 32)}...`);
      ...
      const secondRes = await fetch(endpoint, {
        ...
        timeout: 60000 // Increased to 60s for Testnet stability
      });
