  ...
  initDatabase(dbPath) {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS agent_identities (
        ...
        name TEXT,
        type TEXT DEFAULT 'autonomous_agent',
        metadata_url TEXT,
      ...
      CREATE INDEX IF NOT EXISTS idx_rep ON agent_identities(reputation_score);
    `);
  }
      ...
      try {
        ...
        if (data.agents && data.agents.length > 0) {
          agentsBody.innerHTML = data.agents.map(a => {
            const score = a.reputation_score || 0;
            const repClass = score >= 80 ? 'rep-high' : (score >= 40 ? 'rep-med' : 'rep-low');
            return `
              <tr>
                <td><span class="rep-badge ${repClass}">${score}</span></td>
                <td><span style="font-weight:600;color:#00F5FF">${a.name}</span></td>
                <td class="amount">${(a.total_spent || 0).toFixed(2)} USDC</td>
                <td>${a.successful_txs || 0} <span style="color:#22c55e">✓</span> / ${a.failed_txs || 0} <span style="color:#ef4444">✗</span></td>
              </tr>
            `;
          }).join('');
        }
